...
theorem term_sort_bool_terminates (value : Bool) :
    Terminates (VcTermSortRaw.Term.sort_typed (.Bool value)) := by
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  simp [Terminates]

theorem term_sort_int_terminates (value : Std.I64) :
    Terminates (VcTermSortRaw.Term.sort_typed (.Int value)) := by
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  simp [Terminates]

theorem term_sort_string_terminates (value : String) :
    Terminates (VcTermSortRaw.Term.sort_typed (.String value)) := by
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  simp [Terminates]

theorem term_sort_bytes_terminates (value : List Std.U8) :
    Terminates (VcTermSortRaw.Term.sort_typed (.Bytes value)) := by
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  simp [Terminates]

theorem term_sort_range_terminates (value : List Std.I64) :
    Terminates (VcTermSortRaw.Term.sort_typed (.Range value)) := by
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  simp [Terminates]

theorem term_sort_unit_terminates :
    Terminates (VcTermSortRaw.Term.sort_typed .Unit) := by
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  simp [Terminates]

theorem term_sort_null_reference_terminates :
    Terminates (VcTermSortRaw.Term.sort_typed .NullReference) := by
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  simp [Terminates]

theorem term_sort_nominal_reference_terminates
    (className objectName : String) :
    Terminates (VcTermSortRaw.Term.sort_typed (.NominalReference className objectName)) := by
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
...
theorem term_sort_class_literal_terminates (className : String) :
    Terminates (VcTermSortRaw.Term.sort_typed (.ClassLiteral className)) := by
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  simp [Terminates, alloc.string.String.is_empty,
    Str.Insts.AllocBorrowToOwnedString.to_owned]
  split <;> simp
...
theorem term_sort_permission_mask_valid_terminates
    ...
    Terminates (VcTermSortRaw.Term.sort_typed (.PermissionMaskValid mask field)) := by
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  simp [Terminates, alloc.string.String.is_empty,
    Str.Insts.AllocBorrowToOwnedString.to_owned]
  split <;> simp
...
theorem require_sort_terminates_of_term_sort_terminates
    ...
    (childTerminates : Terminates (VcTermSortRaw.Term.sort_typed term)) :
    Terminates (VcTermSortRaw.require_sort term expected context) := by
  rw [VcTermSortRaw.require_sort.eq_def]
  cases observed : VcTermSortRaw.Term.sort_typed term with
...
theorem raw_loop_terminates_of_invariant
    ...
    (state : State) :
    invariant state → Terminates (loop body state) := by
...
theorem all_list_element_sorts_loop_terminates
    (iter : core.slice.iter.Iter VcTermSort.Sort) (result : Bool)
    (childrenTerminate : ∀ child, child ∈ iter.slice.val →
      Terminates (VcTermSortRaw.is_list_element_sort child)) :
    Terminates (VcTermSortRaw.all_list_element_sorts_loop iter result) := by
  unfold VcTermSortRaw.all_list_element_sorts_loop
  exact sort_bool_accumulator_loop_terminates
    VcTermSortRaw.is_list_element_sort
    VcTermSortRaw.all_list_element_sorts_loop.body iter result
    (by
      intro state invariant
      rw [VcTermSortRaw.all_list_element_sorts_loop.body.eq_def]
      exact sort_bool_accumulator_body_terminates
        VcTermSortRaw.is_list_element_sort state.1 state.2 invariant)
    (by
      intro state next continued
      rw [VcTermSortRaw.all_list_element_sorts_loop.body.eq_def] at continued
      exact sort_bool_accumulator_body_preserves_slice
        VcTermSortRaw.is_list_element_sort state.1 next.1 state.2 next.2 continued)
    (by
      intro state next continued
...
theorem all_list_element_sorts_terminates
    ...
    (childrenTerminate : ∀ child, child ∈ elements.val →
      Terminates (VcTermSortRaw.is_list_element_sort child)) :
    Terminates (VcTermSortRaw.all_list_element_sorts elements) := by
  rw [VcTermSortRaw.all_list_element_sorts.eq_def]
  change Terminates
    (VcTermSortRaw.all_list_element_sorts_loop ⟨elements, 0⟩ true)
...
theorem all_finite_dict_key_sorts_loop_terminates
    (iter : core.slice.iter.Iter VcTermSort.Sort) (result : Bool)
    (childrenTerminate : ∀ child, child ∈ iter.slice.val →
      Terminates (VcTermSortRaw.is_finite_dict_key_sort child)) :
    Terminates (VcTermSortRaw.all_finite_dict_key_sorts_loop iter result) := by
  unfold VcTermSortRaw.all_finite_dict_key_sorts_loop
  exact sort_bool_accumulator_loop_terminates
    VcTermSortRaw.is_finite_dict_key_sort
    VcTermSortRaw.all_finite_dict_key_sorts_loop.body iter result
    (by
      intro state invariant
      rw [VcTermSortRaw.all_finite_dict_key_sorts_loop.body.eq_def]
      exact sort_bool_accumulator_body_terminates
        VcTermSortRaw.is_finite_dict_key_sort state.1 state.2 invariant)
    (by
      intro state next continued
      rw [VcTermSortRaw.all_finite_dict_key_sorts_loop.body.eq_def] at continued
      exact sort_bool_accumulator_body_preserves_slice
        VcTermSortRaw.is_finite_dict_key_sort state.1 next.1 state.2 next.2 continued)
    (by
      intro state next continued
...
theorem all_finite_dict_key_sorts_terminates
    ...
    (childrenTerminate : ∀ child, child ∈ elements.val →
      Terminates (VcTermSortRaw.is_finite_dict_key_sort child)) :
    Terminates (VcTermSortRaw.all_finite_dict_key_sorts elements) := by
  rw [VcTermSortRaw.all_finite_dict_key_sorts.eq_def]
  change Terminates
    (VcTermSortRaw.all_finite_dict_key_sorts_loop ⟨elements, 0⟩ true)
...
theorem all_finite_dict_value_sorts_loop_terminates
    (iter : core.slice.iter.Iter VcTermSort.Sort) (result : Bool)
    (childrenTerminate : ∀ child, child ∈ iter.slice.val →
      Terminates (VcTermSortRaw.is_finite_dict_value_sort child)) :
    Terminates (VcTermSortRaw.all_finite_dict_value_sorts_loop iter result) := by
  unfold VcTermSortRaw.all_finite_dict_value_sorts_loop
  exact sort_bool_accumulator_loop_terminates
    VcTermSortRaw.is_finite_dict_value_sort
    VcTermSortRaw.all_finite_dict_value_sorts_loop.body iter result
    (by
      intro state invariant
      rw [VcTermSortRaw.all_finite_dict_value_sorts_loop.body.eq_def]
      exact sort_bool_accumulator_body_terminates
        VcTermSortRaw.is_finite_dict_value_sort state.1 state.2 invariant)
    (by
      intro state next continued
      rw [VcTermSortRaw.all_finite_dict_value_sorts_loop.body.eq_def] at continued
      exact sort_bool_accumulator_body_preserves_slice
        VcTermSortRaw.is_finite_dict_value_sort state.1 next.1 state.2 next.2 continued)
    (by
      intro state next continued
...
theorem all_finite_dict_value_sorts_terminates
    ...
    (childrenTerminate : ∀ child, child ∈ elements.val →
      Terminates (VcTermSortRaw.is_finite_dict_value_sort child)) :
    Terminates (VcTermSortRaw.all_finite_dict_value_sorts elements) := by
  rw [VcTermSortRaw.all_finite_dict_value_sorts.eq_def]
  change Terminates
    (VcTermSortRaw.all_finite_dict_value_sorts_loop ⟨elements, 0⟩ true)
  exact all_finite_dict_value_sorts_loop_terminates
    ⟨elements, 0⟩ true childrenTerminate

def SortPredicatesTerminate (sort : VcTermSort.Sort) : Prop :=
  Terminates (VcTermSortRaw.is_list_element_sort sort) ∧
  Terminates (VcTermSortRaw.is_finite_dict_key_sort sort) ∧
  Terminates (VcTermSortRaw.is_finite_dict_value_sort sort) ∧
  Terminates (VcTermSortRaw.is_variadic_tuple_element_sort sort)
...
theorem sort_predicates_terminate (sort : VcTermSort.Sort) :
  ...
  · intro elements childrenTerminate
    ...
    constructor
    · rw [VcTermSortRaw.is_list_element_sort.eq_def]
      cases observed : VcTermSortRaw.all_list_element_sorts
    ...
    · constructor
      · rw [VcTermSortRaw.is_finite_dict_key_sort.eq_def]
        cases observed : VcTermSortRaw.all_finite_dict_key_sorts
            (VcTermSort.ModelVec.deref elements) with
      ...
      · constructor
        · rw [VcTermSortRaw.is_finite_dict_value_sort.eq_def]
          cases observed : VcTermSortRaw.all_finite_dict_value_sorts
              (VcTermSort.ModelVec.deref elements) with
        ...
        · rw [VcTermSortRaw.is_variadic_tuple_element_sort.eq_def]
          have tupleListTerminates :
              Terminates (VcTermSortRaw.is_list_element_sort (.Tuple elements)) := by
            rw [VcTermSortRaw.is_list_element_sort.eq_def]
            cases observed : VcTermSortRaw.all_list_element_sorts
                (VcTermSort.ModelVec.deref elements) with
            | div => simp [Terminates, observed] at listTerminates
            | fail error => simp [Terminates, observed]
            | ok value => cases value <;> simp [Terminates, observed]
          cases observed : VcTermSortRaw.is_list_element_sort (.Tuple elements) with
          | div => simp [Terminates, observed] at tupleListTerminates
          | fail error => simp [Terminates, observed]
          | ok value => cases value <;> simp [Terminates, observed]
  · intro element childTerminates
    have listTerminates :
        Terminates (VcTermSortRaw.is_list_element_sort (.VariadicTuple element)) := by
      rw [VcTermSortRaw.is_list_element_sort.eq_def]
      cases observed : VcTermSortRaw.is_list_element_sort element with
      | div => exact (childTerminates.1 observed).elim
      | fail error => simp [Terminates, observed]
      | ok value => cases value <;> simp [Terminates, observed]
    have keyTerminates :
        Terminates (VcTermSortRaw.is_finite_dict_key_sort (.VariadicTuple element)) := by
      rw [VcTermSortRaw.is_finite_dict_key_sort.eq_def]
      cases observed : VcTermSortRaw.is_finite_dict_key_sort element with
      | div => exact (childTerminates.2.1 observed).elim
      | fail error => simp [Terminates, observed]
      | ok value => cases value <;> simp [Terminates, observed]
    have valueTerminates :
        Terminates (VcTermSortRaw.is_finite_dict_value_sort (.VariadicTuple element)) := by
      rw [VcTermSortRaw.is_finite_dict_value_sort.eq_def]
      cases observed : VcTermSortRaw.is_finite_dict_value_sort element with
      | div => exact (childTerminates.2.2.1 observed).elim
      | fail error => simp [Terminates, observed]
      | ok value => cases value <;> simp [Terminates, observed]
    refine ⟨listTerminates, keyTerminates, valueTerminates, ?_⟩
    rw [VcTermSortRaw.is_variadic_tuple_element_sort.eq_def]
    cases listObserved : VcTermSortRaw.is_list_element_sort
    ...
    | ok listAccepted =>
        ...
        | false =>
            cases recursiveObserved :
                VcTermSortRaw.is_variadic_tuple_element_sort element with
  ...
  · intro element childTerminates
    have listTerminates :
        Terminates (VcTermSortRaw.is_list_element_sort (.List element)) := by
      rw [VcTermSortRaw.is_list_element_sort.eq_def]
      cases observed : VcTermSortRaw.is_list_element_sort element with
      | div => exact (childTerminates.1 observed).elim
      | fail error => simp [Terminates, observed]
      | ok value => cases value <;> simp [Terminates, observed]
    have valueTerminates :
        Terminates (VcTermSortRaw.is_finite_dict_value_sort (.List element)) := by
      rw [VcTermSortRaw.is_finite_dict_value_sort.eq_def]
      cases observed : VcTermSortRaw.is_finite_dict_value_sort element with
      | div => exact (childTerminates.2.2.1 observed).elim
      | fail error => simp [Terminates, observed]
    ...
    · rw [VcTermSortRaw.is_finite_dict_key_sort.eq_def]
      simp [Terminates]
    · rw [VcTermSortRaw.is_variadic_tuple_element_sort.eq_def]
      cases observed : VcTermSortRaw.is_list_element_sort (.List element) with
      | div => exact (listTerminates observed).elim
      | fail error => simp [Terminates, observed]
      | ok value => cases value <;> simp [Terminates, observed]
  · intro element childTerminates
    have listTerminates :
        Terminates (VcTermSortRaw.is_list_element_sort (.Set element)) := by
      rw [VcTermSortRaw.is_list_element_sort.eq_def]
      cases observed : VcTermSortRaw.is_finite_dict_key_sort element with
      | div => exact (childTerminates.2.1 observed).elim
      | fail error => simp [Terminates, observed]
      | ok value => cases value <;> simp [Terminates, observed]
    have valueTerminates :
        Terminates (VcTermSortRaw.is_finite_dict_value_sort (.Set element)) := by
      rw [VcTermSortRaw.is_finite_dict_value_sort.eq_def]
      cases observed : VcTermSortRaw.is_finite_dict_key_sort element with
      | div => exact (childTerminates.2.1 observed).elim
      | fail error => simp [Terminates, observed]
    ...
    · rw [VcTermSortRaw.is_finite_dict_key_sort.eq_def]
      simp [Terminates]
    · rw [VcTermSortRaw.is_variadic_tuple_element_sort.eq_def]
      cases observed : VcTermSortRaw.is_list_element_sort (.Set element) with
      | div => exact (listTerminates observed).elim
      | fail error => simp [Terminates, observed]
      | ok value => cases value <;> simp [Terminates, observed]
  · intro key value keyTerminates valueTerminates
    have listTerminates :
        Terminates (VcTermSortRaw.is_list_element_sort (.Dict key value)) := by
      rw [VcTermSortRaw.is_list_element_sort.eq_def]
      cases keyObserved : VcTermSortRaw.is_finite_dict_key_sort key with
      ...
      | ok keyAccepted =>
          ...
          | true =>
              cases valueObserved : VcTermSortRaw.is_list_element_sort value with
    ...
    have dictValueTerminates :
        Terminates (VcTermSortRaw.is_finite_dict_value_sort (.Dict key value)) := by
      rw [VcTermSortRaw.is_finite_dict_value_sort.eq_def]
      cases keyObserved : VcTermSortRaw.is_finite_dict_key_sort key with
      | div => exact (keyTerminates.2.1 keyObserved).elim
      | fail error => simp [Terminates, keyObserved]
      | ok keyAccepted =>
          ...
          | true =>
              cases valueObserved :
                  VcTermSortRaw.is_finite_dict_value_sort value with
    ...
    · rw [VcTermSortRaw.is_finite_dict_key_sort.eq_def]
      simp [Terminates]
    · rw [VcTermSortRaw.is_variadic_tuple_element_sort.eq_def]
      cases observed : VcTermSortRaw.is_list_element_sort (.Dict key value) with
      | div => exact (listTerminates observed).elim
      | fail error => simp [Terminates, observed]
      | ok accepted => cases accepted <;> simp [Terminates, observed]
  · intro key value keyTerminates valueTerminates
    have listTerminates :
        Terminates (VcTermSortRaw.is_list_element_sort (.FiniteDict key value)) := by
      rw [VcTermSortRaw.is_list_element_sort.eq_def]
      cases keyObserved : VcTermSortRaw.is_finite_dict_key_sort key with
      | div => exact (keyTerminates.2.1 keyObserved).elim
      | fail error => simp [Terminates, keyObserved]
      | ok keyAccepted =>
          ...
          | true =>
              ...
              | ok valueAccepted => cases valueAccepted <;>
                  simp [Terminates, keyObserved, valueObserved]
    have dictValueTerminates :
        Terminates (VcTermSortRaw.is_finite_dict_value_sort
    ...
    · rw [VcTermSortRaw.is_variadic_tuple_element_sort.eq_def]
      cases observed :
          VcTermSortRaw.is_list_element_sort (.FiniteDict key value) with
      | div => exact (listTerminates observed).elim
      | fail error => simp [Terminates, observed]
      | ok accepted => cases accepted <;> simp [Terminates, observed]
...
theorem require_all_sorts_body_terminates
    ...
    (childrenTerminate : ∀ child, child ∈ iter.slice.val →
      Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates
      (VcTermSortRaw.require_all_sorts_loop.body expected context iter result) := by
  have nextTerminates := slice_iterator_next_terminates iter
  cases observed : core.slice.iter.IteratorSliceIter.next iter with
  | div => simp [Terminates, observed] at nextTerminates
  | fail error =>
      simp [Terminates, VcTermSortRaw.require_all_sorts_loop.body, observed]
  | ok pair =>
      ...
      | some value =>
          ...
          cases result with
          | Err current =>
              simp [Terminates, VcTermSortRaw.require_all_sorts_loop.body,
          ...
          | Ok checkedUnit =>
              ...
              rw [VcTermSortRaw.require_all_sorts_loop.body]
              rw [observed]
              change Terminates (do
                let cloned ← VcTermSort.Sort.Insts.CoreCloneClone.clone expected
                let checked ← VcTermSortRaw.require_sort value cloned context
                ok (ControlFlow.cont (next, checked)))
              rw [sort_clone_exact]
              cases requiredObserved :
                  VcTermSortRaw.require_sort value expected context with
              | div =>
...
theorem require_all_sorts_loop_terminates
    ...
    (childrenTerminate : ∀ child, child ∈ iter.slice.val →
      Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates
      (VcTermSortRaw.require_all_sorts_loop iter expected context result) := by
...
theorem require_all_sorts_terminates
    ...
    (childrenTerminate : ∀ child, child ∈ values.val →
      Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.require_all_sorts values expected context) := by
  rw [VcTermSortRaw.require_all_sorts.eq_def]
  change Terminates (VcTermSortRaw.require_all_sorts_loop
...
theorem require_variadic_tuple_element_sorts_body_terminates
    ...
    (childrenTerminate : ∀ child, child ∈ iter.slice.val →
      Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.require_variadic_tuple_element_sorts_loop.body
  ...
  | fail error =>
      simp [Terminates,
        VcTermSortRaw.require_variadic_tuple_element_sorts_loop.body, observed]
  | ok pair =>
      obtain ⟨maybeValue, next⟩ := pair
      cases maybeValue with
      | none =>
      ...
      | some value =>
          ...
          | Err current =>
              simp [Terminates,
                ...
                observed, core.result.Result.is_ok]
          | Ok checkedUnit =>
              rw [VcTermSortRaw.require_variadic_tuple_element_sorts_loop.body]
              rw [observed]
              change Terminates (do
                let r ← VcTermSortRaw.Term.sort_typed value
                match r with
              ...
              cases valueObserved : VcTermSortRaw.Term.sort_typed value with
...
theorem require_variadic_tuple_element_sorts_body_preserves_slice
  ...
  cases observed : core.slice.iter.IteratorSliceIter.next iter with
  | fail error =>
      simp [VcTermSortRaw.require_variadic_tuple_element_sorts_loop.body,
...
theorem require_variadic_tuple_element_sorts_loop_terminates
    ...
    (childrenTerminate : ∀ child, child ∈ iter.slice.val →
      Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.require_variadic_tuple_element_sorts_loop
...
theorem require_variadic_tuple_element_sorts_terminates
    ...
    (childrenTerminate : ∀ child, child ∈ values.val →
      Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.require_variadic_tuple_element_sorts
      values expected) := by
  rw [VcTermSortRaw.require_variadic_tuple_element_sorts.eq_def]
  change Terminates (VcTermSortRaw.require_variadic_tuple_element_sorts_loop
...
theorem model_vec_deref_children_terminate_of_smaller
    ...
    ∀ child, child ∈ (VcTermSort.ModelVec.deref values).val →
      Terminates (VcTermSortRaw.Term.sort_typed child) := by
  intro child member
...
theorem collect_sorts_body_terminates
    ...
    (childrenTerminate : ∀ child, child ∈ iter.slice.val →
      Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.collect_sorts_loop.body iter sorts error) := by
  have nextTerminates := slice_iterator_next_terminates iter
  cases observed : core.slice.iter.IteratorSliceIter.next iter with
  | div => simp [Terminates, observed] at nextTerminates
  | fail iteratorError =>
      simp [Terminates, VcTermSortRaw.collect_sorts_loop.body, observed]
  | ok pair =>
      obtain ⟨maybeValue, next⟩ := pair
      cases maybeValue with
      | none =>
      ...
      | some value =>
          ...
          | none =>
              ...
              | fail sortError =>
                  simp [Terminates, VcTermSortRaw.collect_sorts_loop.body,
...
theorem collect_sorts_loop_terminates
    ...
    (childrenTerminate : ∀ child, child ∈ iter.slice.val →
      Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.collect_sorts_loop iter sorts error) := by
...
theorem collect_sorts_terminates
    ...
    (childrenTerminate : ∀ child, child ∈ values.val →
      Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.collect_sorts values) := by
  rw [VcTermSortRaw.collect_sorts.eq_def]
  change Terminates (do
    let output ← VcTermSortRaw.collect_sorts_loop
  ...
  cases observed : VcTermSortRaw.collect_sorts_loop
      ⟨values, 0⟩
      (VcTermSort.ModelVec.with_capacity VcTermSort.Sort
...
theorem validate_int_enum_descriptor_body_terminates
    ...
    (valid : Bool) :
    Terminates
      (VcTermSortRaw.validate_int_enum_descriptor_loop.body
  ...
  | fail iteratorError =>
      simp [Terminates,
        VcTermSortRaw.validate_int_enum_descriptor_loop.body, observed]
  | ok pair =>
...
theorem validate_int_enum_descriptor_loop_terminates
    ...
    (valid : Bool) :
    Terminates
      (VcTermSortRaw.validate_int_enum_descriptor_loop iter names values valid) := by
...
theorem validate_int_enum_descriptor_terminates
    ...
    Terminates (VcTermSortRaw.validate_int_enum_descriptor descriptor) := by
  unfold VcTermSortRaw.validate_int_enum_descriptor
  ...
  · simp only [VcTermSort.ModelVec.is_empty]
    ...
    · have loopTerminates := validate_int_enum_descriptor_loop_terminates
      ...
      cases observed : VcTermSortRaw.validate_int_enum_descriptor_loop
...
theorem require_predicate_argument_sorts_body_terminates
    ...
    (childrenTerminate : ∀ child, child ∈ iter.slice.val →
      Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates
      (VcTermSortRaw.require_predicate_argument_sorts_loop.body iter result) := by
  have nextTerminates := slice_iterator_next_terminates iter
  cases observed : core.slice.iter.IteratorSliceIter.next iter with
  | div => simp [Terminates, observed] at nextTerminates
  | fail iteratorError =>
      simp [Terminates,
        VcTermSortRaw.require_predicate_argument_sorts_loop.body, observed]
  | ok pair =>
      ...
      | some argument =>
          ...
          | Err current =>
              simp [Terminates,
                VcTermSortRaw.require_predicate_argument_sorts_loop.body,
                observed, core.result.Result.is_ok]
          | Ok checkedUnit =>
...
theorem require_predicate_argument_sorts_body_preserves_slice
    ...
    (result nextResult : core.result.Result Unit VcTermSort.SortError)
    (continued :
      VcTermSortRaw.require_predicate_argument_sorts_loop.body iter result =
        .ok (.cont (next, nextResult))) :
    next.slice = iter.slice := by
  cases observed : core.slice.iter.IteratorSliceIter.next iter with
  | fail iteratorError =>
      simp [VcTermSortRaw.require_predicate_argument_sorts_loop.body,
        observed] at continued
  | div =>
      simp [VcTermSortRaw.require_predicate_argument_sorts_loop.body,
        observed] at continued
...
theorem require_predicate_argument_sorts_loop_terminates
    ...
    (childrenTerminate : ∀ child, child ∈ iter.slice.val →
      Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates
      (VcTermSortRaw.require_predicate_argument_sorts_loop iter result) := by
...
theorem require_predicate_argument_sorts_terminates
    ...
    (childrenTerminate : ∀ child, child ∈ arguments.val →
      Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.require_predicate_argument_sorts arguments) := by
  rw [VcTermSortRaw.require_predicate_argument_sorts.eq_def]
  change Terminates
    (VcTermSortRaw.require_predicate_argument_sorts_loop
...
theorem require_permission_transfer_amounts_body_terminates
    ...
    (receiversTerminate : ∀ amount, amount ∈ iter.slice.val →
      Terminates (VcTermSortRaw.Term.sort_typed amount.receiver)) :
    Terminates
      (VcTermSortRaw.require_permission_transfer_amounts_loop.body iter result) := by
  have nextTerminates := slice_iterator_next_terminates iter
  cases observed : core.slice.iter.IteratorSliceIter.next iter with
  | div => simp [Terminates, observed] at nextTerminates
  | fail iteratorError =>
      simp [Terminates,
        VcTermSortRaw.require_permission_transfer_amounts_loop.body, observed]
  | ok pair =>
      ...
      | some amount =>
          ...
          | Err current =>
              simp [Terminates,
                VcTermSortRaw.require_permission_transfer_amounts_loop.body,
                observed, core.result.Result.is_ok]
          | Ok checkedUnit =>
              have requiredTerminates :=
                require_sort_terminates_of_term_sort_terminates
              ...
              cases requiredObserved : VcTermSortRaw.require_sort
                  amount.receiver .Reference
...
theorem require_permission_transfer_amounts_body_preserves_slice
    ...
    (result nextResult : core.result.Result Unit VcTermSort.SortError)
    (continued :
      VcTermSortRaw.require_permission_transfer_amounts_loop.body iter result =
        .ok (.cont (next, nextResult))) :
    next.slice = iter.slice := by
  cases observed : core.slice.iter.IteratorSliceIter.next iter with
  | fail iteratorError =>
...
theorem require_permission_transfer_amounts_loop_terminates
    ...
    (receiversTerminate : ∀ amount, amount ∈ iter.slice.val →
      Terminates (VcTermSortRaw.Term.sort_typed amount.receiver)) :
    Terminates
      (VcTermSortRaw.require_permission_transfer_amounts_loop iter result) := by
...
theorem require_permission_transfer_amounts_terminates
    ...
    (receiversTerminate : ∀ amount, amount ∈ amounts.val →
      Terminates (VcTermSortRaw.Term.sort_typed amount.receiver)) :
    Terminates (VcTermSortRaw.require_permission_transfer_amounts amounts) := by
  rw [VcTermSortRaw.require_permission_transfer_amounts.eq_def]
  change Terminates
    (VcTermSortRaw.require_permission_transfer_amounts_loop
...
theorem all_nominal_reference_keys_body_terminates
    ...
    (result : Bool) :
    Terminates (VcTermSortRaw.all_nominal_reference_keys_loop.body iter result) := by
  have nextTerminates := slice_iterator_next_terminates iter
  cases observed : core.slice.iter.IteratorSliceIter.next iter with
  | div => simp [Terminates, observed] at nextTerminates
  | fail error =>
      simp [Terminates,
        VcTermSortRaw.all_nominal_reference_keys_loop.body, observed]
  | ok pair =>
      obtain ⟨maybeEntry, next⟩ := pair
      cases maybeEntry with
      | none =>
...
theorem all_nominal_reference_keys_loop_terminates
    (iter : core.slice.iter.Iter (VcTermSort.Term × VcTermSort.Term))
    (result : Bool) :
    Terminates (VcTermSortRaw.all_nominal_reference_keys_loop iter result) := by
  unfold VcTermSortRaw.all_nominal_reference_keys_loop
  apply raw_loop_terminates_of_invariant
    (fun state : core.slice.iter.Iter
        (VcTermSort.Term × VcTermSort.Term) × Bool =>
      VcTermSortRaw.all_nominal_reference_keys_loop.body state.1 state.2)
    (fun _ => True)
    (fun state => sliceIteratorRemaining state.1)
  · intro state _
...
theorem all_nominal_reference_keys_terminates
    ...
    Terminates (VcTermSortRaw.all_nominal_reference_keys entries) := by
  unfold VcTermSortRaw.all_nominal_reference_keys
  change Terminates
    (VcTermSortRaw.all_nominal_reference_keys_loop ⟨entries, 0⟩ true)
...
theorem require_finite_dict_entry_sorts_body_terminates
    ...
    (childrenTerminate : ∀ entry, entry ∈ iter.slice.val →
      Terminates (VcTermSortRaw.Term.sort_typed entry.1) ∧
      Terminates (VcTermSortRaw.Term.sort_typed entry.2)) :
    Terminates (VcTermSortRaw.require_finite_dict_entry_sorts_loop.body
  ...
  | fail error =>
      simp [Terminates,
        VcTermSortRaw.require_finite_dict_entry_sorts_loop.body, observed]
  | ok pair =>
      obtain ⟨maybeEntry, next⟩ := pair
      cases maybeEntry with
      | none =>
      ...
      | some entry =>
          ...
          | Err current =>
              simp [Terminates,
                ...
                observed, core.result.Result.is_ok]
          | Ok checkedUnit =>
              rw [VcTermSortRaw.require_finite_dict_entry_sorts_loop.body]
              rw [observed]
              change Terminates (do
                let keyClone ←
                  VcTermSort.Sort.Insts.CoreCloneClone.clone keySort
                let keyChecked ← VcTermSortRaw.require_sort key keyClone
              ...
              | ok keyChecked =>
                  ...
                  | Ok keyUnit =>
                      rw [sort_clone_exact]
                      cases valueObserved : VcTermSortRaw.require_sort value valueSort
...
theorem require_finite_dict_entry_sorts_body_preserves_slice
  ...
  cases observed : core.slice.iter.IteratorSliceIter.next iter with
  | fail error =>
      simp [VcTermSortRaw.require_finite_dict_entry_sorts_loop.body,
        observed] at continued
  | div =>
      simp [VcTermSortRaw.require_finite_dict_entry_sorts_loop.body,
        observed] at continued
...
theorem require_finite_dict_entry_sorts_loop_terminates
    ...
    (childrenTerminate : ∀ entry, entry ∈ iter.slice.val →
      Terminates (VcTermSortRaw.Term.sort_typed entry.1) ∧
      Terminates (VcTermSortRaw.Term.sort_typed entry.2)) :
    Terminates (VcTermSortRaw.require_finite_dict_entry_sorts_loop
...
theorem require_finite_dict_entry_sorts_terminates
    ...
    (childrenTerminate : ∀ entry, entry ∈ entries.val →
      Terminates (VcTermSortRaw.Term.sort_typed entry.1) ∧
      Terminates (VcTermSortRaw.Term.sort_typed entry.2)) :
    Terminates (VcTermSortRaw.require_finite_dict_entry_sorts
      entries keySort valueSort) := by
  rw [VcTermSortRaw.require_finite_dict_entry_sorts.eq_def]
  change Terminates (VcTermSortRaw.require_finite_dict_entry_sorts_loop
...
theorem term_sort_runtime_class_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.RuntimeClass value) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.RuntimeClass value)) := by
...
theorem term_sort_not_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.Not value) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.Not value)) := by
...
theorem term_sort_negate_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.Negate value) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.Negate value)) := by
...
theorem term_sort_string_length_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.StringLength value) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.StringLength value)) := by
...
theorem term_sort_bytes_length_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.BytesLength value) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.BytesLength value)) := by
...
theorem term_sort_class_subtype_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.ClassSubtype actual expected) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.ClassSubtype actual expected)) := by
...
theorem term_sort_implies_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.Implies left right) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.Implies left right)) := by
...
theorem term_sort_less_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.Less left right) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.Less left right)) := by
...
theorem term_sort_less_equal_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.LessEqual left right) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.LessEqual left right)) := by
...
theorem term_sort_greater_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.Greater left right) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.Greater left right)) := by
...
theorem term_sort_greater_equal_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.GreaterEqual left right) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.GreaterEqual left right)) := by
...
theorem term_sort_add_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.Add left right) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.Add left right)) := by
...
theorem term_sort_subtract_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.Subtract left right) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.Subtract left right)) := by
...
theorem term_sort_multiply_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.Multiply left right) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.Multiply left right)) := by
...
theorem term_sort_bytes_get_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.BytesGet bytes index) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.BytesGet bytes index)) := by
...
theorem term_sort_permission_positive_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.PermissionPositive mask receiver field) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.PermissionPositive mask receiver field)) := by
...
theorem term_sort_floor_divide_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.FloorDivideByPositive value divisor) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.FloorDivideByPositive value divisor)) := by
  have valueSmaller :
  ...
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  cases valueObserved :
      VcTermSortRaw.require_sort value .Int VcTermSort.SortContext.FloorDivisionOperand with
...
theorem term_sort_list_length_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.ListLength value) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.ListLength value)) := by
...
theorem term_sort_list_slice_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.ListSlice source lower upper step) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates
      (VcTermSortRaw.Term.sort_typed (.ListSlice source lower upper step)) := by
  have sourceSmaller :
  ...
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  cases sourceObserved : VcTermSortRaw.Term.sort_typed source with
...
theorem term_sort_list_concat_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.ListConcat left right) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.ListConcat left right)) := by
  ...
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  cases leftObserved : VcTermSortRaw.Term.sort_typed left with
  ...
  | ok leftChecked =>
      ...
      | Ok leftSort =>
          ...
          case List leftElement =>
            have rightTerminates := smallerTerminates right rightSmaller
            cases rightObserved : VcTermSortRaw.Term.sort_typed right with
...
theorem term_sort_list_sum_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.ListSum source) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.ListSum source)) := by
...
theorem term_sort_list_sorted_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.ListSorted source) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.ListSorted source)) := by
...
theorem term_sort_set_length_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.SetLength value) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.SetLength value)) := by
...
theorem term_sort_dict_length_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.DictLength value) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.DictLength value)) := by
...
theorem term_sort_field_read_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.FieldRead mask receiver field sort) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.FieldRead mask receiver field sort)) := by
  have receiverSmaller :
  ...
  cases receiverObserved :
      VcTermSortRaw.require_sort receiver .Reference VcTermSort.SortContext.FieldReceiver with
  ...
  | ok checked =>
      ...
      | Ok checkedUnit =>
          ...
          case List element =>
            ...
            cases predicateObserved : VcTermSortRaw.is_list_element_sort element with
...
theorem term_sort_permission_at_least_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child
        ...
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed
  ...
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  cases receiverObserved :
      VcTermSortRaw.require_sort receiver .Reference VcTermSort.SortContext.PermissionReceiver with
  | div => simp [Terminates, receiverObserved] at receiverRequired
  | fail error => simp [Terminates, receiverObserved]
  | ok checked =>
      cases checked with
...
theorem term_sort_equal_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.Equal left right) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.Equal left right)) := by
...
theorem term_sort_if_then_else_terminates_of_smaller
  ...
  cases conditionObserved :
      VcTermSortRaw.require_sort condition .Bool VcTermSort.SortContext.ConditionalGuard with
  ...
  | ok conditionChecked =>
      ...
      | Err error =>
          simp [Terminates, conditionObserved,
            ...
            core.result.Result.Insts.CoreOpsTryTraitFromResidualResultInfallible.from_residual]
      | Ok checkedUnit =>
          cases thenObserved : VcTermSortRaw.Term.sort_typed thenValue with
          ...
          | ok thenChecked =>
              ...
              | Err error =>
                  simp [Terminates, conditionObserved, thenObserved,
                    ...
                    core.result.Result.Insts.CoreOpsTryTraitFromResidualResultInfallible.from_residual]
              | Ok thenSort =>
                  cases elseObserved : VcTermSortRaw.Term.sort_typed elseValue with
...
theorem term_sort_list_get_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.ListGet list index) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.ListGet list index)) := by
  ...
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  cases indexObserved :
      VcTermSortRaw.require_sort index .Int VcTermSort.SortContext.ListIndex with
  ...
  | ok indexChecked =>
      ...
      | Err error =>
          simp [Terminates, indexObserved,
            ...
            core.result.Result.Insts.CoreOpsTryTraitFromResidualResultInfallible.from_residual]
      | Ok checkedUnit =>
          cases listObserved : VcTermSortRaw.Term.sort_typed list with
          | div => simp [Terminates, listObserved] at listTerminates
...
theorem term_sort_list_contains_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.ListContains list value) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.ListContains list value)) := by
  ...
  | ok listChecked =>
      ...
      | Ok listSort =>
          ...
          case List element =>
            ...
            cases valueObserved :
                VcTermSortRaw.require_sort value element
...
theorem term_sort_set_contains_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.SetContains set value) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.SetContains set value)) := by
  ...
  have valueTerminates := smallerTerminates value valueSmaller
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  cases setObserved : VcTermSortRaw.Term.sort_typed set with
...
theorem term_sort_dict_contains_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.DictContains dict key) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.DictContains dict key)) := by
  ...
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  cases dictObserved : VcTermSortRaw.Term.sort_typed dict with
  ...
  | ok dictChecked =>
      ...
      | Ok dictSort =>
          ...
          case Dict expected valueSort =>
            ...
            cases keyObserved :
                VcTermSortRaw.require_sort key expected
...
theorem term_sort_dict_get_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.DictGet dict key) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.DictGet dict key)) := by
...
theorem term_sort_int_enum_projection_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.IntEnumProjection value) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.IntEnumProjection value)) := by
  have valueSmaller : TermStrictlySmaller value (.IntEnumProjection value) := by
    simp [TermStrictlySmaller]
  have valueTerminates := smallerTerminates value valueSmaller
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.IntEnumProjection value)]
  simp only
  ...
  case IntEnumValue descriptor payload =>
    cases observed :
        VcTermSortRaw.Term.sort_typed (.IntEnumValue descriptor payload) with
    | div => simp [Terminates, observed] at valueTerminates
    | fail error => simp [Terminates, observed]
    | ok checked =>
        cases checked <;>
...
theorem term_sort_int_enum_domain_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.IntEnumDomain value) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.IntEnumDomain value)) := by
  have valueSmaller : TermStrictlySmaller value (.IntEnumDomain value) := by
    simp [TermStrictlySmaller]
  have valueTerminates := smallerTerminates value valueSmaller
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.IntEnumDomain value)]
  simp only
  rw [box_as_ref_refines]
  cases value <;>
    simp [Terminates, Str.Insts.AllocBorrowToOwnedString.to_owned]
...
theorem term_sort_int_enum_identity_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.IntEnumIdentity left right) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.IntEnumIdentity left right)) := by
  have leftSmaller :
  ...
  have leftTerminates := smallerTerminates left leftSmaller
  have rightTerminates := smallerTerminates right rightSmaller
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.IntEnumIdentity left right)]
  simp only
  rw [box_as_ref_refines]
  cases left
  case IntEnumValue leftDescriptor leftPayload =>
    ...
    case IntEnumValue rightDescriptor rightPayload =>
      cases leftObserved : VcTermSortRaw.Term.sort_typed
      ...
      | ok leftChecked =>
          ...
          | Err error =>
              simp [Terminates, leftObserved,
                ...
                core.result.Result.Insts.CoreOpsTryTraitFromResidualResultInfallible.from_residual]
          | Ok leftSort =>
              cases rightObserved : VcTermSortRaw.Term.sort_typed
...
theorem term_sort_int_enum_value_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.IntEnumValue descriptor value) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.IntEnumValue descriptor value)) := by
  have valueSmaller :
  ...
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.IntEnumValue descriptor value)]
  simp only
  cases descriptorObserved :
      VcTermSortRaw.validate_int_enum_descriptor descriptor with
...
theorem term_sort_variable_terminates
    ...
    Terminates (VcTermSortRaw.Term.sort_typed (.Variable variableName sort)) := by
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.Variable variableName sort)]
  simp only
  cases sort
...
theorem term_sort_tuple_get_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.TupleGet tuple index) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.TupleGet tuple index)) := by
  ...
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.TupleGet tuple index)]
  simp only
  cases observed : VcTermSortRaw.Term.sort_typed tuple with
...
theorem term_sort_and_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.And values) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.And values)) := by
  ...
  have requiredTerminates := require_all_sorts_terminates
    ...
    childrenTerminate
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.And values)]
  simp only
  exact require_sort_then_ok_terminates _ requiredTerminates .Bool
...
theorem term_sort_or_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.Or values) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.Or values)) := by
  have childrenSmaller : ∀ child,
      child ∈ (show List VcTermSort.Term from values) →
  ...
  have requiredTerminates := require_all_sorts_terminates
    (VcTermSort.ModelVec.deref values) .Bool VcTermSort.SortContext.BooleanOperand
    childrenTerminate
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.Or values)]
  simp only
  exact require_sort_then_ok_terminates _ requiredTerminates .Bool
...
theorem term_sort_string_concat_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.StringConcat values) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.StringConcat values)) := by
  have childrenSmaller : ∀ child,
      child ∈ (show List VcTermSort.Term from values) →
  ...
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.StringConcat values)]
  simp only
...
theorem term_sort_bytes_concat_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.BytesConcat values) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.BytesConcat values)) := by
  have childrenSmaller : ∀ child,
      child ∈ (show List VcTermSort.Term from values) →
  ...
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.BytesConcat values)]
  simp only
...
theorem sort_clone_require_all_then_list_ok_terminates
    ...
    (requiredTerminates : Terminates
      (VcTermSortRaw.require_all_sorts values elementSort context)) :
    Terminates (do
      let cloned ← VcTermSort.Sort.Insts.CoreCloneClone.clone elementSort
      let r ← VcTermSortRaw.require_all_sorts values cloned context
      let cf ← core.result.Result.Insts.CoreOpsTry.branch r
      match cf with
      | core.ops.control_flow.ControlFlow.Continue _ =>
...
theorem sort_clone_require_all_then_dict_keys_ok_terminates
    ...
    (context : VcTermSort.SortContext)
    (requiredTerminates : Terminates
      (VcTermSortRaw.require_all_sorts values keySort context)) :
    Terminates (do
...
theorem term_sort_list_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.List elementSort values) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.List elementSort values)) := by
  have childrenSmaller : ∀ child,
      child ∈ (show List VcTermSort.Term from values) →
  ...
  have requiredTerminates := require_all_sorts_terminates
    ...
    childrenTerminate
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.List elementSort values)]
  simp only
  have predicateTerminates := is_list_element_sort_terminates elementSort
  cases predicateObserved : VcTermSortRaw.is_list_element_sort elementSort with
  | div => simp [Terminates, predicateObserved] at predicateTerminates
  | fail error => simp [Terminates, predicateObserved]
  | ok accepted =>
      cases accepted with
...
theorem term_sort_variadic_tuple_terminates_of_smaller
  ...
  simp only
  cases predicateObserved :
      VcTermSortRaw.is_variadic_tuple_element_sort elementSort with
  | div => simp [Terminates, predicateObserved] at predicateTerminates
  | fail error => simp [Terminates, predicateObserved]
  | ok accepted =>
      cases accepted with
      ...
      | true =>
          cases requiredObserved :
              VcTermSortRaw.require_variadic_tuple_element_sorts
...
theorem term_sort_variadic_tuple_length_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.VariadicTupleLength value) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.VariadicTupleLength value)) := by
  have valueTerminates := smallerTerminates value (by
    simp [TermStrictlySmaller])
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.VariadicTupleLength value)]
  simp only
  cases observed : VcTermSortRaw.Term.sort_typed value with
  | div => simp [Terminates, observed] at valueTerminates
  | fail error => simp [Terminates, observed]
  | ok checked =>
      cases checked with
...
theorem term_sort_variadic_tuple_get_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.VariadicTupleGet tuple index) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.VariadicTupleGet tuple index)) := by
  ...
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.VariadicTupleGet tuple index)]
  simp only
  cases indexObserved : VcTermSortRaw.require_sort index .Int
  ...
  | ok indexChecked =>
      ...
      | Err error =>
          simp [Terminates, indexObserved,
            ...
            core.result.Result.Insts.CoreOpsTryTraitFromResidualResultInfallible.from_residual]
      | Ok unit =>
          cases tupleObserved : VcTermSortRaw.Term.sort_typed tuple with
...
theorem term_sort_variadic_tuple_slice_terminates_of_smaller
  ...
  simp only
  cases observed : VcTermSortRaw.Term.sort_typed source with
...
theorem term_sort_tuple_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.Tuple values) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.Tuple values)) := by
  have childrenSmaller : ∀ child,
      child ∈ (show List VcTermSort.Term from values) →
  ...
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.Tuple values)]
  simp only
  cases observed : VcTermSortRaw.collect_sorts
...
theorem term_sort_predicate_instance_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.PredicateInstance predicate arguments) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates
      (VcTermSortRaw.Term.sort_typed (.PredicateInstance predicate arguments)) := by
  have childrenSmaller : ∀ child,
  ...
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.PredicateInstance predicate arguments)]
  simp only
  ...
  · by_cases argumentsEmpty : arguments.isEmpty
    ...
    · simp [alloc.string.String.is_empty, predicateEmpty,
        VcTermSort.ModelVec.is_empty, argumentsEmpty]
      cases observed : VcTermSortRaw.require_predicate_argument_sorts
...
theorem term_sort_dict_keys_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      TermStrictlySmaller child (.DictKeys keySort values) →
        Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (VcTermSortRaw.Term.sort_typed (.DictKeys keySort values)) := by
  have childrenSmaller : ∀ child,
      child ∈ (show List VcTermSort.Term from values) →
  ...
  have requiredTerminates := require_all_sorts_terminates
    ...
    childrenTerminate
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.DictKeys keySort values)]
  simp only
  have predicateTerminates := is_finite_dict_key_sort_terminates keySort
  cases predicateObserved : VcTermSortRaw.is_finite_dict_key_sort keySort with
  | div => simp [Terminates, predicateObserved] at predicateTerminates
  | fail error => simp [Terminates, predicateObserved]
  | ok accepted =>
      cases accepted with
...
theorem term_sort_finite_dict_terminates_of_smaller
  ...
  have childrenTerminate : ∀ entry,
      ...
      Terminates (VcTermSortRaw.Term.sort_typed entry.1) ∧
      Terminates (VcTermSortRaw.Term.sort_typed entry.2) := by
    intro entry member
  ...
  have acceptedKeyTerminates : Terminates (do
      let valueValid ← VcTermSortRaw.is_finite_dict_value_sort valueSort
      if valueValid
      then
        let checked ← VcTermSortRaw.require_finite_dict_entry_sorts
    ...
    cases valueObserved : VcTermSortRaw.is_finite_dict_value_sort valueSort with
    ...
    | ok valueValid =>
        ...
        | true =>
            cases requiredObserved : VcTermSortRaw.require_finite_dict_entry_sorts
  ...
  have tailTerminates (exactReferenceKeys : Bool) : Terminates (do
      let keyValid ← VcTermSortRaw.is_finite_dict_key_sort keySort
      if keyValid
      then
        let valueValid ← VcTermSortRaw.is_finite_dict_value_sort valueSort
        if valueValid
    ...
    cases keyObserved : VcTermSortRaw.is_finite_dict_key_sort keySort with
  ...
  rw [VcTermSortRaw.Term.sort_typed.eq_def (.FiniteDict keySort valueSort entries)]
  simp only
...
theorem term_sort_permission_mask_transition_terminates_of_smaller
  ...
  have consumedTerminate : ∀ amount,
      amount ∈ (VcTermSort.ModelVec.deref consumed).val →
        Terminates (VcTermSortRaw.Term.sort_typed amount.receiver) := by
    intro amount member
  ...
  · simp [alloc.string.String.is_empty]
    ...
    · simp [Terminates, Str.Insts.AllocBorrowToOwnedString.to_owned]
    ·
      cases consumedObserved : VcTermSortRaw.require_permission_transfer_amounts
      ...
      | ok consumedChecked =>
          ...
          | Ok checkedUnit =>
              cases producedObserved :
                  VcTermSortRaw.require_permission_transfer_amounts
...
theorem validate_bound_occurrences_one_terminates_of_validate
    ...
    Terminates
      (VcTermSortRaw.validate_bound_occurrences.one term binder binderSort) := by
...
theorem validate_bound_occurrences_two_terminates_of_validate
    ...
    (leftValidated : Terminates
      (VcTermSortRaw.validate_bound_occurrences left binder binderSort))
    (rightValidated : Terminates
      (VcTermSortRaw.validate_bound_occurrences right binder binderSort)) :
    Terminates
  ...
  cases leftObserved : VcTermSortRaw.validate_bound_occurrences.one
...
theorem validate_bound_occurrences_all_body_terminates
    ...
    (childrenValidate : ∀ child, child ∈ iter.slice.val →
      Terminates (VcTermSortRaw.validate_bound_occurrences child binder binderSort)) :
    Terminates
  ...
  | fail error =>
      simp [Terminates,
        VcTermSortRaw.validate_bound_occurrences.all_loop.body, observed]
  | ok pair =>
      ...
      | some term =>
          ...
          | Ok checkedUnit =>
              ...
              rw [VcTermSortRaw.validate_bound_occurrences.all_loop.body]
              rw [observed]
...
theorem validate_bound_occurrences_all_body_preserves_slice
  ...
  cases observed : core.slice.iter.IteratorSliceIter.next iter with
  | fail error =>
      simp [VcTermSortRaw.validate_bound_occurrences.all_loop.body,
        observed] at continued
  | div =>
      simp [VcTermSortRaw.validate_bound_occurrences.all_loop.body,
        observed] at continued
...
theorem validate_bound_occurrences_all_terminates
    ...
    (childrenValidate : ∀ child, child ∈ terms.val →
      Terminates (VcTermSortRaw.validate_bound_occurrences child binder binderSort)) :
    Terminates
      (VcTermSortRaw.validate_bound_occurrences.all terms binder binderSort) := by
...
theorem validate_bound_occurrences_all_transfers_body_terminates
    ...
    (receiversValidate : ∀ amount, amount ∈ iter.slice.val →
      Terminates (VcTermSortRaw.validate_bound_occurrences
  ...
  | ok pair =>
      ...
      | some amount =>
          ...
          | Ok checkedUnit =>
              ...
              rw [VcTermSortRaw.validate_bound_occurrences.all_transfers_loop.body]
              rw [observed]
              cases validatedObserved :
                  VcTermSortRaw.validate_bound_occurrences.one
  ...
  | ok pair =>
      ...
      | some entry =>
          ...
          | Ok checkedUnit =>
              ...
              rw [VcTermSortRaw.validate_bound_occurrences.all_entries_loop.body]
              rw [observed]
              cases validatedObserved :
                  VcTermSortRaw.validate_bound_occurrences.two
...
theorem validate_bound_occurrences_terminates_of_smaller
    ...
    Terminates
      (VcTermSortRaw.validate_bound_occurrences term binder binderSort) := by
      ...
      have elseValidated :=
        validate_bound_occurrences_one_terminates_of_validate
          elseValue binder binderSort (smallerValidate elseValue (by
            simp [TermStrictlySmaller] <;> omega))
      cases conditionObserved : VcTermSortRaw.validate_bound_occurrences.one
      ...
      | ok checked =>
          ...
          | Err error =>
              simp [Terminates,
                ...
                core.result.Result.Insts.CoreOpsTryTraitFromResidualResultInfallible.from_residual]
          | Ok checkedUnit =>
              cases thenObserved : VcTermSortRaw.validate_bound_occurrences.one
  ...
  | PermissionMaskTransition preMask postMask field consumed produced =>
      ...
      have producedValidated : Terminates
        ...
        exact permission_receiver_strictly_smaller amount produced
          (.PermissionMaskTransition preMask postMask field consumed produced)
          sourceMember (by simp <;> omega)
      cases consumedObserved : VcTermSortRaw.validate_bound_occurrences.all_transfers
  ...
  | SetComprehension resultName source nested nestedSort mapped filter =>
      ...
      ·
        ...
        have filterValidated : ∀ child, filter = some child →
            Terminates (VcTermSortRaw.validate_bound_occurrences.one
          ...
          subst filter
          simp [TermStrictlySmaller] <;> omega
        cases sourceObserved : VcTermSortRaw.validate_bound_occurrences.one
        ...
        | ok sourceChecked =>
            ...
            | Err error =>
                simp [Terminates,
                  ...
                  core.result.Result.Insts.CoreOpsTryTraitFromResidualResultInfallible.from_residual]
            | Ok checkedUnit =>
                cases mappedObserved : VcTermSortRaw.validate_bound_occurrences.one
        ...
        | ok sourceChecked =>
            ...
            | Err error =>
                simp [Terminates,
                  ...
                  core.result.Result.Insts.CoreOpsTryTraitFromResidualResultInfallible.from_residual]
            | Ok checkedUnit =>
                cases pairObserved : VcTermSortRaw.validate_bound_occurrences.two
                    key value binder binderSort with
...
theorem require_bound_term_sort_terminates
    ...
    (termSortTerminates : Terminates (VcTermSortRaw.Term.sort_typed term)) :
    Terminates
  ...
  have validated := validate_bound_occurrences_terminates
    term binder binderSort
  cases validateObserved : VcTermSortRaw.validate_bound_occurrences
      term binder binderSort with
...
theorem require_comprehension_body_terminates
    ...
    (mappedSortTerminates : Terminates (VcTermSortRaw.Term.sort_typed mapped))
  ...
  ·
    ...
    cases mappedObserved : VcTermSortRaw.require_bound_term_sort
    ...
    | ok checked =>
        ...
        | Ok checkedUnit =>
            ...
            | some filterTerm =>
                ...
                simp only [core.result.Result.Insts.CoreOpsTry.branch, bind_tc_ok]
                exact result_try_then_ok_unit_terminates
                  (VcTermSortRaw.require_bound_term_sort binder binderSort
...
theorem all_nominal_references_body_terminates
    ...
    Terminates (VcTermSortRaw.all_nominal_references_loop.body iter result) := by
  have nextTerminates := slice_iterator_next_terminates iter
  cases observed : core.slice.iter.IteratorSliceIter.next iter with
  | div => simp [Terminates, observed] at nextTerminates
  | fail error =>
      simp [Terminates, VcTermSortRaw.all_nominal_references_loop.body, observed]
  | ok pair =>
      obtain ⟨maybeTerm, next⟩ := pair
      cases maybeTerm with
      | none =>
          simp [Terminates, VcTermSortRaw.all_nominal_references_loop.body,
            observed]
      | some term =>
          cases result with
          | false =>
          ...
          | true =>
              cases term <;>
                simp [Terminates,
                  VcTermSortRaw.all_nominal_references_loop.body, observed]

theorem all_nominal_references_loop_terminates
    (iter : core.slice.iter.Iter VcTermSort.Term) (result : Bool) :
    Terminates (VcTermSortRaw.all_nominal_references_loop iter result) := by
  unfold VcTermSortRaw.all_nominal_references_loop
  apply raw_loop_terminates_of_invariant
    (fun state : core.slice.iter.Iter VcTermSort.Term × Bool =>
      VcTermSortRaw.all_nominal_references_loop.body state.1 state.2)
...
theorem all_nominal_references_terminates
    (values : Slice VcTermSort.Term) :
    Terminates (VcTermSortRaw.all_nominal_references values) := by
  unfold VcTermSortRaw.all_nominal_references
  simp only [SharedSlice.Insts.CoreIterTraitsCollectIntoIteratorSharedIter.into_iter,
    bind_tc_ok]
...
theorem is_exact_nominal_reference_list_terminates
    (source : VcTermSort.Term) :
    Terminates (VcTermSortRaw.is_exact_nominal_reference_list source) := by
  cases source <;> simp [VcTermSortRaw.is_exact_nominal_reference_list, Terminates]
...
theorem is_reference_binder_projection_terminates
    ...
    Terminates (VcTermSortRaw.is_reference_binder_projection mapped binder) := by
  cases mapped <;>
    simp [VcTermSortRaw.is_reference_binder_projection, Terminates,
      string_eq_refines]
...
theorem term_sort_for_all_terminates_of_smaller
    ...
    (smallerTerminates : ∀ child,
      ...
      Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates
      (VcTermSortRaw.Term.sort_typed (.ForAll binder binderSort body)) := by
  rw [VcTermSortRaw.Term.sort_typed.eq_def]
  simp [alloc.string.String.is_empty]
  split
  · simp [Terminates, Str.Insts.AllocBorrowToOwnedString.to_owned]
  ·
    ...
    exact require_sort_then_ok_terminates
      (VcTermSortRaw.require_bound_term_sort binder binderSort body
  ...
  | ok result =>
      ...
      | Ok sourceSort =>
          ...
          case List sourceElement =>
            ...
            cases requiredObserved : VcTermSortRaw.require_comprehension_body
...
theorem require_comprehension_then_set_terminates
    ...
    (requiredTerminates : Terminates
      (VcTermSortRaw.require_comprehension_body
        binder sourceElement mapped elementSort filter)) :
    Terminates (do
      let r ← VcTermSortRaw.require_comprehension_body
        binder sourceElement mapped elementSort filter
      let cf ← core.result.Result.Insts.CoreOpsTry.branch r
      match cf with
      | core.ops.control_flow.ControlFlow.Continue _ =>
      ...
      | core.ops.control_flow.ControlFlow.Break residual =>
          core.result.Result.Insts.CoreOpsTryTraitFromResidualResultInfallible.from_residual
            VcTermSort.Sort (core.convert.FromSame VcTermSort.SortError) residual) := by
  cases observed : VcTermSortRaw.require_comprehension_body
      binder sourceElement mapped elementSort filter with
  | div => simp [Terminates, observed] at requiredTerminates
  | fail error => simp [Terminates]
  | ok checked =>
...
theorem set_comprehension_finish_terminates
    ...
    (filterSortTerminates : ∀ child, filter = some child →
      Terminates (VcTermSortRaw.Term.sort_typed child)) :
    Terminates (do
      let valid ← VcTermSortRaw.is_finite_dict_key_sort elementSort
      if valid then
        let r ← VcTermSortRaw.require_comprehension_body
          binder sourceElement mapped elementSort filter
        let cf ← core.result.Result.Insts.CoreOpsTry.branch r
...
theorem set_comprehension_reference_prelude_terminates
    ...
    Terminates (do
      let exactList ← VcTermSortRaw.is_exact_nominal_reference_list source
      let exactProjection ←
        if exactList then
  ...
  apply result_bind_terminates
    (input := VcTermSortRaw.is_exact_nominal_reference_list source)
  ...
  · intro exactList
    ...
    | true =>
        apply result_bind_terminates
          (input := VcTermSortRaw.is_reference_binder_projection mapped binder)
...
theorem set_comprehension_reference_prelude_ok_shape
    ...
    binder1 = binder ∧ elementSort1 = elementSort ∧ mapped1 = mapped ∧
      filter1 = filter := by
  cases exactObserved : VcTermSortRaw.is_exact_nominal_reference_list source with
  ...
  | ok exactList =>
      ...
      | true =>
          cases projectionObserved :
              VcTermSortRaw.is_reference_binder_projection mapped binder with
              ...
              | true =>
                  ...
                  · intro inputs preludeObserved
                    ...
                    have shape : binder1 = binder ∧
                        ...
                        filter1 = filter := by
                      cases exactObserved :
                          VcTermSortRaw.is_exact_nominal_reference_list source with
                      ...
                      | ok exactList =>
                          ...
                          | true =>
                              cases projectionObserved :
                                  VcTermSortRaw.is_reference_binder_projection
                                    mapped binder with
                              | div =>
...
theorem require_dict_comprehension_chain_terminates
    ...
    (valueRequiredTerminates : Terminates
      (VcTermSortRaw.require_bound_term_sort binder sourceElement value valueSort
    ...
    Terminates (do
      ...
      match cf with
      | core.ops.control_flow.ControlFlow.Continue _ =>
          let r1 ← VcTermSortRaw.require_bound_term_sort
      ...
      | core.ops.control_flow.ControlFlow.Break residual =>
          core.result.Result.Insts.CoreOpsTryTraitFromResidualResultInfallible.from_residual
            VcTermSort.Sort (core.convert.FromSame VcTermSort.SortError) residual) := by
  cases keyObserved : VcTermSortRaw.require_comprehension_body
  ...
  | ok keyChecked =>
      ...
      | Err error =>
          simp [Terminates,
            ...
            core.result.Result.Insts.CoreOpsTryTraitFromResidualResultInfallible.from_residual]
      | Ok checkedUnit =>
          cases valueObserved : VcTermSortRaw.require_bound_term_sort
...
theorem term_sort_terminates (term : VcTermSort.Term) :
    Terminates (VcTermSortRaw.Term.sort_typed term) := by
  apply term_strictly_smaller_well_founded.induction term
...
theorem term_sort_extraction_entrypoint_terminates (term : VcTermSort.Term) :
    Terminates (VcTermSortRaw.term_sort_extraction_entrypoint term) := by
  unfold VcTermSortRaw.term_sort_extraction_entrypoint
